...
public class GitHubSourceAPIHttpClient {

    private static final Logger log = LoggerFactory.getLogger(GitHubSourceAPIHttpClient.class);
    ...
    public GitHubSourceAPIHttpClient(GitHubSourceConnectorConfig config){
        this.config = config;
    }

    protected JSONArray getNextEvents(Integer page, Instant since) throws InterruptedException {

        HttpResponse<JsonNode> jsonResponse;
        try {
            jsonResponse = getNextEventsAPI(page,since);
            ...
            switch (jsonResponse.getStatus()){
                ...
                case 403:
                    ...
                    return getNextEvents(page,since);
                default:
    ...
    protected HttpResponse<JsonNode> getNextEventsAPI(Integer page,Instant since) throws UnirestException {
    ...
    protected String constructUrl(Integer page, Instant since){
        return String.format(
                "https://api.github.com/events?page=%s&per_page=%s&since=%s&state=all&direction=asc",
                page,
                config.getBatchSize(),
                since.toString());
    }
